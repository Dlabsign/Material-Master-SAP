...
CASE lv_action.
  ...
  WHEN 'GET_PENDING_LIST' OR 'GET_REQUEST_LIST'.
    ...
    LOOP AT lt_bp_all INTO ls_bp_rec.
      ...
      IF sy-subrc = 0 AND ls_tb003t-rltxt IS NOT INITIAL.
        ls_out-bp_role_text   = CONV #( ls_tb003t-rltxt ).
      ELSEIF ls_bp_rec-bp_role CS 'CU' OR ls_bp_rec-bu_group CP 'C*'.
        ls_out-bp_role_text   = 'Customer (FLCU01 & FLCU00)'.
      ELSE.
        ls_out-bp_role_text   = 'Vendor (FLVN01 & FLVN00)'.
      ENDIF.
  ...
  " ====================================================================
  " 2. GET BP COUNTERS (FOR BANK STEWARD PERSPECTIVE)
  " ====================================================================
  WHEN 'GET_COUNTERS'.
    ...
    " Records checked by Bank Steward or approved
    SELECT COUNT( * ) FROM zmdg_bp_req
  ...
  " ====================================================================
  " 4. BANK STEWARD APPROVAL / CHECKED -> STATUS = 'CHECKED'
  "    Prerequisite: Must already be approved by Data Steward ('DATA_CHECKED')
  " ====================================================================
  WHEN 'MARK_AS_CHECKED' OR 'CHECK_AND_FORWARD' OR 'SUBMIT_TO_APPROVAL'
    OR 'APPROVE_BANK' OR 'CHECK_BP'.
    lv_req_no = request->get_form_field( 'REQ_NO' ).
    IF lv_req_no IS INITIAL.
      lv_req_no = request->get_form_field( 'REQ_ID' ).
    ENDIF.
    ...
    " Cegah duplicate approval oleh Bank Steward
    IF ls_bp_check-stw_bank_status = 'X'.
      lv_json = |\{"status":"ERROR","message":|
             && |"Permohonan { lv_bp_act_key } sudah disetujui oleh Bank Steward sebelumnya."\}|.
      _m_response->set_content_type( 'application/json' ).
      _m_response->set_cdata( lv_json ).
      _m_navigation->response_complete( ).
      RETURN.
    ...
    ELSE.
      " Bank Steward telah menyetujui, masih menunggu Data Steward
      lv_new_status = 'BANK_CHECKED'.
    ENDIF.
    ...
    IF lv_new_status = 'CHECKED'.
      lv_json = |\{"status":"SUCCESS","message":|
             && |"Business Partner { lv_bp_act_key } berhasil disetujui Bank Steward! Kedua steward telah memverifikasi, diteruskan ke Final Approval."\}|.
    ELSE.
      lv_json = |\{"status":"SUCCESS","message":|
             && |"Business Partner { lv_bp_act_key } berhasil disetujui Bank Steward (menunggu verifikasi Data Steward)."\}|.
    ENDIF.
    _m_response->set_content_type( 'application/json' ).
    _m_response->set_cdata( lv_json ).
    _m_navigation->response_complete( ).
  ...
  WHEN 'RETURN_TO_REQUESTOR' OR 'REJECT'.
    ...
    lv_json = |\{"status":"SUCCESS","message":|
           && |"Business Partner Request { lv_bp_rej_k } telah ditolak oleh Bank Steward."\}|.
    _m_response->set_content_type( 'application/json' ).
    _m_response->set_cdata( lv_json ).
    _m_navigation->response_complete( ).
    RETURN.
