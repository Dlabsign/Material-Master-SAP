...
CASE lv_action.
  ...
  " ==================================================================
  WHEN 'AI_CHECK_DUPLICATE'.
    TYPES: BEGIN OF ty_ai_candidate,
             matnr       TYPE string,
             maktx       TYPE string,
             source      TYPE string,
             match_level TYPE string,
           END OF ty_ai_candidate.

    TYPES: BEGIN OF ty_ai_res_item,
             row_index             TYPE i,
             original_text         TYPE string,
             status                TYPE string, " UNIQUE, POSSIBLE_DUPLICATE, CONFIRMED_DUPLICATE, RECOMMENDATION
             recommended_name      TYPE string,
             recommendation_reason TYPE string,
             candidates            TYPE TABLE OF ty_ai_candidate WITH DEFAULT KEY,
           END OF ty_ai_res_item.

    DATA: lt_ai_results     TYPE TABLE OF ty_ai_res_item,
          ls_ai_res         TYPE ty_ai_res_item,
          ls_cand           TYPE ty_ai_candidate,
          lv_single_txt     TYPE string,
          lv_like_pat       TYPE string,
          lt_mara_cand      TYPE TABLE OF ty_mara_res,
          ls_mara_cand      TYPE ty_mara_res,
          lv_stg_req        TYPE zmdg_req_dtl-req_no,
          lv_stg_maktx      TYPE zmdg_req_dtl-maktx,
          gc_gemini_api_key TYPE string VALUE 'AQ.Ab8RN6Jtzglttkrlq3udMBqIYWIojDclLHwP5_3gdTstbUynqw',
          lv_ai_url          TYPE string,
          lo_http_client     TYPE REF TO if_http_client,
          lv_ai_payload      TYPE string,
          lv_ai_response     TYPE string.

    lv_single_txt = request->get_form_field( 'MAKTX' ).
    IF lv_single_txt IS NOT INITIAL.
      CLEAR ls_ai_res.
      ls_ai_res-row_index     = 1.
      ls_ai_res-original_text = lv_single_txt.
      ls_ai_res-status        = 'UNIQUE'.

      " 1. Exact Match Check (MARA & Staging)
      TRANSLATE lv_single_txt TO UPPER CASE.
      CONDENSE lv_single_txt.

      SELECT SINGLE a~matnr, b~maktx
        FROM mara AS a
        INNER JOIN makt AS b ON a~matnr = b~matnr AND b~spras = @sy-langu
        WHERE b~maktx = @lv_single_txt
        INTO (@ls_cand-matnr, @ls_cand-maktx).

      IF sy-subrc = 0.
        ls_cand-source      = 'SAP MARA (Master)'.
        ls_cand-match_level = 'High (Exact)'.
        APPEND ls_cand TO ls_ai_res-candidates.
        ls_ai_res-status    = 'CONFIRMED_DUPLICATE'.
      ENDIF.

      " Exact Match in Staging
      SELECT SINGLE req_no, maktx
        FROM zmdg_req_dtl
        WHERE maktx = @lv_single_txt
        INTO (@lv_stg_req, @lv_stg_maktx).

      IF sy-subrc = 0.
        CLEAR ls_cand.
        ls_cand-matnr       = lv_stg_req.
        ls_cand-maktx       = lv_stg_maktx.
        ls_cand-source      = 'Staging Request'.
        ls_cand-match_level = 'High (Exact Request)'.
        APPEND ls_cand TO ls_ai_res-candidates.
        IF ls_ai_res-status <> 'CONFIRMED_DUPLICATE'.
          ls_ai_res-status = 'CONFIRMED_DUPLICATE'.
        ENDIF.
      ENDIF.

      " 2. Normalized & Like Pattern Match if not exact
      IF ls_ai_res-status <> 'CONFIRMED_DUPLICATE'.
        CONCATENATE '%' lv_single_txt '%' INTO lv_like_pat.

        SELECT a~matnr, b~maktx, a~mtart, a~matkl, a~meins, a~bismt, a~mbrsh, a~spart
          FROM mara AS a
          LEFT OUTER JOIN makt AS b ON a~matnr = b~matnr AND b~spras = @sy-langu
          WHERE b~maktx LIKE @lv_like_pat
          INTO TABLE @lt_mara_cand
          UP TO 10 ROWS.

        IF lt_mara_cand IS NOT INITIAL.
          ls_ai_res-status = 'POSSIBLE_DUPLICATE'.
          LOOP AT lt_mara_cand INTO ls_mara_cand.
            CLEAR ls_cand.
            ls_cand-matnr       = ls_mara_cand-matnr.
            ls_cand-maktx       = ls_mara_cand-maktx.
            ls_cand-source      = 'SAP MARA (Similar)'.
            ls_cand-match_level = 'Medium'.
            APPEND ls_cand TO ls_ai_res-candidates.
          ENDLOOP.
        ENDIF.
      ENDIF.

      " 3. Call Google AI Studio (Gemini 2.5 Flash API) via ABAP HTTP Client
      CONCATENATE 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key='
                  gc_gemini_api_key INTO lv_ai_url.
      ...
      IF sy-subrc = 0 AND lo_http_client IS BOUND.
        ...
        CONCATENATE '{"contents":[{"parts":[{"text":"Standardize this SAP Material Description according to SAP naming conventions (Type + Object + Specification): '
                    lv_single_txt
                    '. Return only the uppercase description."}]}]}' INTO lv_ai_payload.

        lo_http_client->request->set_cdata( lv_ai_payload ).

        lo_http_client->send( EXCEPTIONS OTHERS = 1 ).
        IF sy-subrc = 0.
          lo_http_client->receive( EXCEPTIONS OTHERS = 1 ).
          IF sy-subrc = 0.
            lv_ai_response = lo_http_client->response->get_cdata( ).
          ENDIF.
        ENDIF.
        lo_http_client->close( ).
      ENDIF.

      " Standardized Recommendation Generator
      ls_ai_res-recommended_name      = lv_single_txt.
      ls_ai_res-recommendation_reason = 'Nama diubah ke huruf kapital sesuai standar SAP.'.

      APPEND ls_ai_res TO lt_ai_results.
    ENDIF.

    lv_json = /ui2/cl_json=>serialize( data = lt_ai_results compress = 'X' pretty_name = /ui2/cl_json=>pretty_mode-low_case ).
    _m_response->set_content_type( 'application/json' ).
    _m_response->set_cdata( lv_json ).
    navigation->goto_page( '' ).
    RETURN.
