  ...
  TYPES: BEGIN OF ty_req_list,
          ...
          req_no      TYPE string,
          matnr       TYPE string,
          old_maktx   TYPE string,
          new_maktx   TYPE string,
          werks       TYPE string,
          mtart       TYPE string,
          remarks     TYPE string,
  ...
  CASE lv_action.
    ...
    " ==================================================================
    " 1. GET ALL REQUESTS (CREATE & CHANGE MATERIAL)
    " ==================================================================
    WHEN 'GET_PENDING_LIST'.
      CLEAR: lt_hdr_db, lt_req_list.

      " 1A. Ambil Data Pengajuan Material Baru (Request Create)
      ...
      LOOP AT lt_hdr_db INTO ls_hdr_db.
        ...
        SELECT COUNT( * ) FROM zmdg_req_dtl INTO @ls_req_list-total_item
          WHERE req_no = @ls_hdr_db-req_no.

        SELECT COUNT( * ) FROM zmdg_req_dtl
          WHERE req_no = @ls_hdr_db-req_no AND matnr_ext IS NOT INITIAL
          INTO @ls_req_list-coded_count.

        APPEND ls_req_list TO lt_req_list.
      ENDLOOP.

      " 1B. Ambil Data Pengajuan Perubahan Material (Request Change)
      DATA: lt_chg_h_db TYPE TABLE OF zmdg_chg_hdr,
            ls_chg_h_db TYPE zmdg_chg_hdr,
            lt_chg_d_db TYPE TABLE OF zmdg_chg_dtl,
            ls_chg_d_db TYPE zmdg_chg_dtl,
            lv_diff_txt TYPE string,
            lv_disp_mat TYPE mara-matnr,
            lv_prev_cnt TYPE i,
            lv_rem_chg  TYPE i.

      SELECT * FROM zmdg_chg_hdr
        INTO TABLE @lt_chg_h_db
        ORDER BY req_date DESCENDING, req_time DESCENDING.

      LOOP AT lt_chg_h_db INTO ls_chg_h_db.
        CLEAR: ls_req_list, lv_diff_txt.
        ls_req_list-req_type    = 'CHANGE'.
        ls_req_list-req_no      = CONV #( ls_chg_h_db-chg_req_no ).
        lv_disp_mat             = ls_chg_h_db-matnr.
        SHIFT lv_disp_mat LEFT DELETING LEADING '0'.
        ls_req_list-matnr       = CONV #( lv_disp_mat ).
        ls_req_list-old_maktx   = CONV #( ls_chg_h_db-old_maktx ).
        ls_req_list-new_maktx   = CONV #( ls_chg_h_db-new_maktx ).
        ls_req_list-werks       = CONV #( ls_chg_h_db-werks ).
        ls_req_list-mtart       = CONV #( ls_chg_h_db-mtart ).
        ls_req_list-sub_reason  = CONV #( ls_chg_h_db-change_reason ).
        ls_req_list-remarks     = |Material Change: { ls_req_list-matnr }|.
        ls_req_list-req_date    = CONV #( ls_chg_h_db-req_date ).
        ls_req_list-req_time    = CONV #( ls_chg_h_db-req_time ).
        ls_req_list-requestor   = CONV #( ls_chg_h_db-requestor ).
        ls_req_list-status      = CONV #( ls_chg_h_db-status ).
        ls_req_list-rej_reason  = CONV #( ls_chg_h_db-rej_reason ).
        ls_req_list-total_item  = ls_chg_h_db-total_changes.
        ls_req_list-coded_count = ls_chg_h_db-total_changes.

        " Preview snippet ringkasan perubahan field
        CLEAR: lt_chg_d_db, lv_prev_cnt.
        SELECT * FROM zmdg_chg_dtl
          INTO TABLE @lt_chg_d_db
          WHERE chg_req_no = @ls_chg_h_db-chg_req_no.

        SORT lt_chg_d_db BY item_no ASCENDING.

        LOOP AT lt_chg_d_db INTO ls_chg_d_db.
          lv_prev_cnt = lv_prev_cnt + 1.
          IF lv_prev_cnt > 2.
            EXIT.
          ENDIF.
          IF lv_diff_txt IS NOT INITIAL.
            lv_diff_txt = |{ lv_diff_txt }; |.
          ENDIF.
          lv_diff_txt = |{ lv_diff_txt }{ ls_chg_d_db-field_label }: { ls_chg_d_db-old_value } -> { ls_chg_d_db-new_value }|.
        ENDLOOP.
        IF ls_chg_h_db-total_changes > 2.
          lv_rem_chg = ls_chg_h_db-total_changes - 2.
          lv_diff_txt = |{ lv_diff_txt } (+{ lv_rem_chg } more)|.
        ENDIF.
        ls_req_list-change_diff = lv_diff_txt.

        APPEND ls_req_list TO lt_req_list.
      ENDLOOP.

      " Urutkan seluruh antrean berdasarkan tanggal dan waktu terbaru
      SORT lt_req_list BY req_date DESCENDING req_time DESCENDING.
    ...
    " ==================================================================
    " 2. GET REQUEST ITEMS DETAIL (CREATE MATERIAL)
    " ==================================================================
    ...
    " ==================================================================
    " 2B. GET CHANGE REQUEST DETAIL & VALUE COMPARISON
    " ==================================================================
    WHEN 'GET_CHANGE_DETAIL'.
      lv_req_no = request->get_form_field( 'REQ_NO' ).
      IF lv_req_no IS INITIAL.
        lv_req_no = request->get_form_field( 'CHG_REQ_NO' ).
      ENDIF.

      DATA: ls_chg_h_det TYPE zmdg_chg_hdr,
            lt_chg_d_det TYPE TABLE OF zmdg_chg_dtl.

      SELECT SINGLE * FROM zmdg_chg_hdr
        INTO @ls_chg_h_det
        WHERE chg_req_no = @lv_req_no.

      SELECT * FROM zmdg_chg_dtl
        INTO TABLE @lt_chg_d_det
        WHERE chg_req_no = @lv_req_no
        ORDER BY item_no ASCENDING.

      TYPES: BEGIN OF ty_chg_detail_resp,
               status  TYPE string,
               header  TYPE zmdg_chg_hdr,
               changes TYPE TABLE OF zmdg_chg_dtl WITH DEFAULT KEY,
             END OF ty_chg_detail_resp.
      DATA: ls_chg_resp TYPE ty_chg_detail_resp.

      ls_chg_resp-status  = 'SUCCESS'.
      ls_chg_resp-header  = ls_chg_h_det.
      ls_chg_resp-changes = lt_chg_d_det.
    ...
    WHEN 'SUBMIT_TO_APPROVAL' OR 'CHECK_AND_FORWARD'.
      ...
      " --- CEK APAKAH INI MERUPAKAN REQUEST CHANGE MATERIAL ---
      SELECT SINGLE * FROM zmdg_chg_hdr INTO @DATA(ls_check_chg_hdr)
        WHERE chg_req_no = @lv_req_no.
      IF sy-subrc = 0.
        UPDATE zmdg_chg_hdr
          SET status     = 'CHECKED',
        ...
        lv_json = |\{"status":"SUCCESS","message":"Change Request { lv_req_no } telah berhasil dicek dan diteruskan ke Approval!"\}|.
        _m_response->set_content_type( 'application/json' ).
        _m_response->set_cdata( lv_json ).
        navigation->goto_page( '' ).
        RETURN.
      ...
      LOOP AT lt_check_dtl INTO ls_check_dtl.
        ...
        " C13. MRP Group (DISGR) check in T438X for Plant
        IF ls_check_dtl-disgr IS NOT INITIAL AND ls_check_dtl-werks IS NOT INITIAL.
          SELECT SINGLE disgr FROM t438x INTO @DATA(lv_dum_disgr)
            WHERE werks = @ls_check_dtl-werks AND disgr = @ls_check_dtl-disgr.
          IF sy-subrc <> 0.
            IF lv_item_err IS NOT INITIAL. lv_item_err = |{ lv_item_err }; |. ENDIF.
            lv_item_err = |{ lv_item_err }MRP Group '{ ls_check_dtl-disgr }' tdk terdaftar utk Plant '{ ls_check_dtl-werks }' (T438X)|.
          ENDIF.
        ENDIF.

        " C14. Origin Group (HERBL / HRKFT) check in TKKH1
        IF ls_check_dtl-herbl IS NOT INITIAL.
          SELECT SINGLE hrkft FROM tkkh1 INTO @DATA(lv_dum_herbl)
            WHERE hrkft = @ls_check_dtl-herbl.
          IF sy-subrc <> 0.
            IF lv_item_err IS NOT INITIAL. lv_item_err = |{ lv_item_err }; |. ENDIF.
            lv_item_err = |{ lv_item_err }Origin Group '{ ls_check_dtl-herbl }' tdk terdaftar (TKKH1)|.
          ENDIF.
        ENDIF.
    ...
    WHEN 'RETURN_TO_REQUESTOR' OR 'REJECT'.
      ...
      " --- CEK APAKAH INI MERUPAKAN REQUEST CHANGE MATERIAL ---
      SELECT SINGLE * FROM zmdg_chg_hdr INTO @DATA(ls_rej_chg_hdr)
        WHERE chg_req_no = @lv_req_no.
      IF sy-subrc = 0.
        UPDATE zmdg_chg_hdr
          SET status     = 'REJECTED',
        ...
        " Cari alamat email user requestor jika ada
        DATA: lv_chg_reqor TYPE usr21-bname,
              lt_chg_smtp  TYPE TABLE OF bapiadsmtp,
              ls_chg_smtp  TYPE bapiadsmtp,
              lt_chg_ret   TYPE TABLE OF bapiret2,
              lv_chg_email TYPE string.

        lv_chg_reqor = ls_rej_chg_hdr-requestor.
        IF lv_chg_reqor IS NOT INITIAL.
          CALL FUNCTION 'BAPI_USER_GET_DETAIL'
            EXPORTING
              username = lv_chg_reqor
            TABLES
              return   = lt_chg_ret
              addsmtp  = lt_chg_smtp.

          READ TABLE lt_chg_smtp INTO ls_chg_smtp INDEX 1.
          IF sy-subrc = 0 AND ls_chg_smtp-e_mail IS NOT INITIAL.
            lv_chg_email = ls_chg_smtp-e_mail.
          ENDIF.
        ENDIF.

        IF lv_chg_email IS NOT INITIAL.
          TRY.
              DATA: lo_chg_send TYPE REF TO cl_bcs,
                    lo_chg_doc  TYPE REF TO cl_document_bcs,
                    lo_chg_rec  TYPE REF TO if_recipient_bcs,
                    lt_chg_body TYPE bcsy_text.

              lo_chg_send = cl_bcs=>create_persistent( ).
              APPEND |Yth. { ls_rej_chg_hdr-requestor },| TO lt_chg_body.
              APPEND | | TO lt_chg_body.
              APPEND |Permohonan Perubahan Material ({ lv_req_no }) telah DITOLAK:| TO lt_chg_body.
              APPEND |Material        : { ls_rej_chg_hdr-matnr }| TO lt_chg_body.
              APPEND |Alasan Penolakan: { lv_reason }| TO lt_chg_body.
              APPEND |Data Steward    : { sy-uname }| TO lt_chg_body.

              lo_chg_doc = cl_document_bcs=>create_document(
                              i_type    = 'RAW'
                              i_text    = lt_chg_body
                              i_subject = CONV #( |[MDG REJECTED] Change Request { lv_req_no } Ditolak| ) ).
              lo_chg_send->set_document( lo_chg_doc ).
              lo_chg_rec = cl_cam_address_bcs=>create_internet_address( CONV #( lv_chg_email ) ).
              lo_chg_send->add_recipient( lo_chg_rec ).
              lo_chg_send->set_send_immediately( abap_true ).
              lo_chg_send->send( i_with_error_screen = abap_false ).
              COMMIT WORK.
            CATCH cx_bcs.
          ENDTRY.
        ENDIF.

        lv_json = |\{"status":"SUCCESS","message":"Change Request { lv_req_no } telah berhasil ditolak dan dikembalikan ke Requestor."\}|.
        _m_response->set_content_type( 'application/json' ).
        _m_response->set_cdata( lv_json ).
        navigation->goto_page( '' ).
        RETURN.
